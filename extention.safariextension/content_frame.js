  ...
  for(var i=0, len=mutations.length; i < len; i++){
    if(mutations[i].addedNodes.length > 0){
      ...
      if(formDom){
        ...
        if(form.type === "signIn"){
          safari.self.tab.dispatchMessage("fillAccountCheck");
        }
      }
    }
  }
...
var dialogFunction = function(theMessageEvent) {
  ...
  }else if(theMessageEvent.name === "confirmChangePasswordBox"){
  ...
  }else if(theMessageEvent.name === "noLoginNotification"){
    saveDialog.message = "現在未ログインのようです。ログインしていただかないとアカウントは登録されません。";
    saveDialog.insert();
  }else if(theMessageEvent.name === "fillAccount"){
    var submitBtn, loginIdElementName, passWordElementName;
    var accountData = theMessageEvent.message;
    for(var i=0, len=accountData.length; i < len; i++){
      saveDialog.select_options += "<option login-id='"+i+"'>"+ accountData[i].loginId +"</option>"
    }
    ...
    if(passWordElementName){
      ...
      document.addEventListener('click', function(e){
        if(e.target.className ==='cushy-ext-submit-js'){
          ...
          var account =  accountData[loginId];
  ...
  }else if(theMessageEvent.name === "autoLogin" || theMessageEvent.name === "analogLogin"){
...
safari.self.addEventListener("message", dialogFunction, false);
