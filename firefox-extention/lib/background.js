...
var autoLoginFunc = function(request,sender,sendResponse){
  ...
  }else if(request.action === 'autoLoginFromWeb'){
    var account = new AccountList();
    account.loginId = request.loginData["loginId"];
    var domain = request.loginData["domain"];
    chrome.storage.local.get(["accounts"], function(result){
      var accounts = result["accounts"][domain];
      if(accounts){
        for(var i=0, len=accounts.length; i < len; i++){
          if(accounts[i].loginId === account.loginId){
            account.password = accounts[i].password;
            break;
          }
        }
        setTimeout(function(){
          client.sendMsg({ action: 'autoLogin', accountData: account});
        }, 500);
      }
    });
  }
